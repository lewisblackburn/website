...
export function ShareIcons({ url, title, className }: { url: string; title: string; className?: string }) {
  return (
    <div className={cn('flex justify-center gap-2', className)}>
      <EmailShareButton url={url} subject={title} body={'Leggi questo post di Lewis Blackburn, ne vale la pena!'}>
      ...
      <LinkedinShareButton
        url={url}
        title={title}
        summary={'Leggi questo post di Lewis Blackburn, ne vale la pena!'}
        source={'Lewis Blackburn'}>
        <div className={buttonVariants({ size: 'icon', variant: 'outline' })}>
