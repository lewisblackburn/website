  ...
  return (
    <Card className={cn('w-full not-prose overflow-hidden rounded-xl', shineAnimation)}>
      <div className='relative'>
        <div
          className={cn('relative z-10 flex flex-col gap-4 p-6', align === 'center' && 'items-center justify-center')}>
          ...
          <div className='flex gap-4'>
            {secondaryButtonText && secondaryButtonUrl && (
              <Button variant='outline' asChild>
                <Link href={UtmUrl(secondaryButtonUrl, { medium: UtmMediums.Blog, content: 'card_cta' })}>
                  {secondaryButtonText}
                </Link>
              </Button>
            )}
